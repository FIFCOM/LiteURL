...
if ($action === 'create') {
    ...
    if (lurlFn::userPermission(lurlFn::apiQueryUser())) {
        $alias = $_REQUEST['alias'] ?? null;
    } else {
        $alias = lurlFn::randStr(LURL_MIN_ALIAS_LENGTH);
    }
    $key = !empty($_REQUEST['key']) && strlen($_REQUEST['key']) > 0 && strlen($_REQUEST['key']) <= 16 ? $_REQUEST['key'] : "1";
    ...
    if ($alias && $uri && $expire) {
        if (lurlFn::userPermission(lurlFn::apiQueryUser())) {
            if (strlen($alias) >= LURL_MIN_ALIAS_LENGTH
                ...
                $lurlSetStatus = lurlFn::set($uri, $alias, "$key", $expire);
                if ($lurlSetStatus) $lurlShortURL = $GLOBALS['lurlScheme'] . $SvrName . '/~' . $alias;
