...
$lurlCustomAlias = isset($_REQUEST['customAlias']) && strlen($_REQUEST['customAlias']) > 0 ? $_REQUEST['customAlias'] : lurl::rand_str(LURL_MIN_ALIAS_LENGTH);
...
if (!lurl::userPermission("test")) $lurlNeverExpireStatus = "disabled"; else $lurlNeverExpireStatus = "";
...
if (!$lurlCustomAlias || !$lurlCustomUri || !$lurlCustomExpire) {
    ...
    if (lurl::userPermission("test")) {
        if (strlen($lurlCustomAlias) >= LURL_MIN_ALIAS_LENGTH
            && strlen($lurlCustomUri) < 2048
            && strlen($lurlCustomKey) < 16) {
            $lurlSetStatus = lurl::set($lurlCustomUri, $lurlCustomAlias, "$lurlCustomKey", $lurlCustomExpire);
